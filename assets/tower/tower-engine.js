...
function mountScrollWorld(container, config) {
  ...
  SECTIONS.forEach((s, i) => {
    ...
    c.innerHTML =
      ...
      (s.tags && s.tags.length ? `<ul class="sw-copy__tags">${s.tags.map(t => (t && t.href) ? `<li><a href="${esc(t.href)}">${esc(t.label)}</a></li>` : `<li>${esc(t)}</li>`).join('')}</ul>` : '') +
  ...
  function loadClip(s) {
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        ...
        if (s.loopPlay) { v.loop = true; v.setAttribute('loop', ''); }
        ...
        v.addEventListener('loadeddata', () => {
          if (s.loopPlay) { if (s.visible && !reduce) { try { v.play(); } catch (e) {} } }
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      if (s.loopPlay && s.video && s.ready) {
        if (s.visible && s.video.paused && !reduce) { try { s.video.play(); } catch (e) {} }
        else if (!s.visible && !s.video.paused) { try { s.video.pause(); } catch (e) {} }
      }
  ...
  function raf() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      if (s.loopPlay) continue;   // ambient loops play on their own clock, never scrubbed
...
function injectCSS() {
  ...
  .sw-copy__tags{list-style:none;display:flex;flex-wrap:wrap;gap:8px;margin:24px 0 0;padding:0;pointer-events:auto;}
  .sw-copy__tags li a{color:inherit;text-decoration:none;display:block;}
  .sw-copy__tags li:hover{transform:translateY(-1px);box-shadow:0 4px 12px color-mix(in srgb,var(--sw-accent) 25%,transparent);}
  .sw-copy__tags li{transition:transform .2s,box-shadow .2s;}
